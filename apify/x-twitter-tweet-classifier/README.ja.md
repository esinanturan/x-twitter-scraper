...
Xquikは世界最速かつ最安値のX(Twitter)スクレイパーサービスで、最も網羅的なXデータを提供します。X (Twitter) Tweet Classifierは、あなた自身のラベル、スコア、はい/いいえの質問にすべてのポストで回答します。他のApify Actorはすべて、フィルタリングや重複排除の前に課金します。Xquikは、配信済みでユニークかつフィルタ条件に一致する結果にのみ課金します。

自分自身の質問でX(Twitter)のポストを分類し、元のポストデータをそのまま保持します。**X (Twitter) Tweet Classifier with AI Analysis**は、一致するポストを収集し、ポストごとに1～8個の型付き質問に回答します。サポートのトリアージ用のカテゴリ、優先度付け用のスコア、関連性用の確率などです。プリセットには、ブランド監視、苦情、競合他社、購入意向、製品フィードバック、ニュース、感情、市場心理が用意されており、カスタム質問でそれらを置き換えることもできます。

- 確率、確信度、質問のバージョンを備えた**型付きの回答**。
- **あなたの質問、あなたのカテゴリ**: 質問ごとに最大255個のカテゴリ。
...
各実行は、key-value storeに`analysis-summary`レコードを書き込み、実行レポートの`results.analysisSummary`の下にも同じ内容を記録します。これは、分析済み、失敗、スキップされた行の数を数え、エンゲージメントを合計し、すべての質問を要約します。カスタム質問ごとに個別のブロックが用意されます。choiceの質問にはカテゴリごとの件数と割合、scoreの質問には平均値とレベルごとの件数、はい/いいえの質問にははい・いいえの件数です。数値は小数点以下4桁に丸められます。結果がゼロの実行では、カウントはゼロ、平均は`null`としてレポートされます。カスタム質問の代わりに組み込みのレンズを実行するには、`analysis.preset`に`brand`、`complaints`、`purchase_intent`、`product_feedback`、`competitors`、`sentiment`、`market`、`news`のいずれかを渡してください。この場合、サマリーはそのレンズを質問ごとにレポートします。各行には、リンク先のホスト名である`sourceDomains`と、テキスト内で見つかった`$NVDA`のような`cashtags`も一覧表示されます。また、`monitor.baselineDatasetId`が設定されている場合、サマリーの`monitor`ブロックが比較ステータスを数え、変更のあった行を最大50件まで一覧表示します。
...
はい。各質問に付けた`version`はすべての回答とともに保存されるため、時間をかけて質問を改善していく中で、どの表現がどの結果を生んだかを追跡できます。
