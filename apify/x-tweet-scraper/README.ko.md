...
이후 체크포인트를 남길 수 있습니다. 서비스가 정체된 페이지네이션을
보고하면 실행은 가져온 행을 유지하고 그 대상에 바로 체크포인트를
남깁니다. 체크포인트가 남은 실행은 불완전한 추출을
...
상한에 도달하거나 적격 데이터를 모두 쓸 때까지 계속 실행합니다. 호출자는 여전히
유한한 Apify 시간 제한을 설정할 수 있습니다. 그러면 `completionReason:
...
Actor는 트윗 URL을 최대 100개 단위의 동시 배치로 조회합니다. 부분 성공 응답은
...
작동합니다. Actor는 과금 전에 각 행을 검증합니다. 하한은 포함됩니다.
