...
ビルドを指定しない場合、ApifyはこのActorの`latest`のデフォルトを使用します。コンソールでの実行と標準的なAPIの例は、そのデフォルトを引き継ぎます。
...
リポストの行は`isRetweet`を`true`に設定します。その`text`には元のポストの全文が入り、`retweeted_tweet`には元のポストとその著者、各種カウントが入ります。
...
行に閲覧者固有の状態が含まれることはありません。フォロー、ブロック、ミュート、ブックマーク、いいね、リポスト、編集権限などの閲覧者フラグは、rawの出力を含めて常に削除されます。
...
別の実行に費用をかける前に、空の結果になった理由を確認してください。レポートと最終的な診断情報の`filtering`オブジェクトは、フィルタで除外された行数を示します。`serverFilteredRows`、`actorFilteredRows`、`pagesWithUnknownServerFiltering`を確認してください。フィルタで除外された行に結果課金は発生しません。
...
`failedSubtargets`は、エラーの後に停止したクエリとプロフィール対象の数を数えます。配信済みの行はデータセットに残り、課金の対象になります。エラーは対象が存在しないことを意味しません。これらの実行は`completionReason: "partial_failure"`を使用します。
...
`completionReason: "pagination_safety_limit"`は読み取り失敗ではありません。実行は有効な行を保持したうえで、新しい結果を返さなくなった対象を終了しました。実行は抽出が未完了であることを報告します。`failedSubtargets`は`0`のままです。支払いは配信済みの行に対してのみ発生します。

デフォルトのApifyタイムアウトは`0`なので、実行に時間制限はありません。Actorは、上限に達するか、対象データを使い尽くすまで継続します。有限のApifyタイムアウトを設定することもできます。その場合、`completionReason: "deadline_reached"`は、その上限が近づいていることを意味します。Actorは上限の前に行とレポートを保存し、正常に終了します。配信済みの行は1回だけ課金されます。
...
ポストのURLは、そのポストを重複なく入力順に返します。プロフィールのURLは、そのアカウントのポストを返します。検索のURLは、そのクエリを実行します。リストのURLは、そのリストのポストを返します。`maxItems`は、貼り付けたすべてのURLの結果全体に上限を設けます。
...
各ハンドルは、そのアカウントのポストを返します。Actorは、出力と課金の前に重複行を除去します。ユーザー名には任意で`@`プレフィックスを付けられます。ハンドルとプロフィールのURLは、XのPostsタブと同じくリポストを残します。日付やフィルタを指定しても同じです。リポストを除くには`tweetTypes.excludeRetweets`を設定してください。
...
`from:elonmusk since:2026-01-01 until:2026-01-02`のような、日付範囲を指定したアカウントの過去データ取得にも対応します。各検索語は、それぞれの`searchTerm`の帰属を保持します。`maxItems`は、すべての検索語の結果全体に上限を設けます。Actorは、返されたすべてのポストを`since:`、`until:`、Unix時間の範囲と照合します。フィルタ付きの検索は、一致する結果が見つかるか、Xに結果がなくなるまで読み取りを続けます。
...
結果は入力順を保ち、重複を除き、指定したポストのみを含みます。
...
`profileTweets`は、X上のプロフィールのPostsタブに従います。アカウントのポスト、リポスト、自分のポストへのリプライを日付順で返します。他のアカウントへのリプライと他の著者の会話コンテキストは、課金前に除外されます。

オリジナルのポストだけが必要な場合は、不要な種類を除外してください。
...
`tweetTypes.excludeReplies`、`excludeRetweets`、`excludeQuotes`は、すべての取得元で機能します。検索では、これらを`-filter:replies`、`-filter:nativeretweets`、`-filter:quote`としてXに送ります。プロフィールやリストでは、Actor自身がそれらの行を破棄します。除外された行はデータセットに届きません。そのため、その行に支払うことはなく、`maxItems`にも数えられません。

`profileReplies`は、XのWith Repliesタブに従います。対象の著者本人が作成したプロフィールのポストとリプライを返します。Actorは、他の著者による会話コンテキストを除外します。リプライのみが必要な場合は、`filter:replies`や`to:`検索を使用してください。

検索とページネーションが必要なポストのモードは、`time.since`、`time.until`、Unixタイムスタンプ、`lang`をサポートします。対象は、プロフィールのPosts、With Replies、Media、Likes、リスト、リプライ、引用、スレッドです。対応するフラットな日付演算子も使えます。Actorは課金前に各行を確認します。日付の下限は含み、上限は含みません。日付フィルタは、使える日付のない行を除外します。言語フィルタは、言語が欠落しているか一致しない行を除外します。フィルタで除外された行が、要求した結果の上限を消費することはありません。日付範囲を指定したリストの実行は、古い日付にもすばやく到達し、下限を過ぎると終了します。リストのかなり古い範囲では、一部のリプライが欠けることがあります。上限は含まないため、`since`と`until`に同じ日付を指定すると空の範囲になります。丸1日を取得するには、`until`を翌日に設定してください。ポストのフィルタは、ユーザーリストや、ポスト・記事の直接検索には適用されません。

`time.withinTime`と`within_time`も同じモードで機能します。`7d`を指定すると、実行が読み取りを始める時点までの直近7日間のポストを残します。2006年より前までさかのぼる期間では、すべてのポストを残します。

`mode: "replies"`はより厳格です。すべてのポスト行には、要求されたポストIDと一致する`inReplyToId`があります。ネストされた会話内のリプライは、直接のリプライとしてカウントされません。Xが報告よりも少ないリプライしか表示しない場合、Actorは見つかった行を保持します。上限に達していない場合は、`diagnostics`に1件の`replies-incomplete`レコードを追加します。実行は、上限に達するか、Xにリプライがなくなるまで部分的なままです。`replyCoverage`は、リプライ件数とカバレッジの詳細をレポートします。1つのリプライ対象について25,000件を超える合計を含め、要求する合計に`maxItems`を設定してください。
...
フラットなポストの行はすべて`media`を持ちます。メディアのないポストは空のリストを持つため、スプレッドシートでも型付きパイプラインでも、どの行も同じキーを持ちます。
...
`queryType: "Latest + Top"`を設定すると、Xの2つの検索モードを1回の実行で使えます。Actorは課金前に重複を除き、どちらかのモードの結果で上限まで埋めます。`Top`は関連性順で、網羅的ではありません。`includeSearchTerms: true`を設定すると、一致した各クエリが`searchTerm`フィールドとして付加されます。
...
新しい連携では標準フィールドを使用してください。上の移行表にある別名は引き続き利用できます。`includeRaw`は`outputVariant: "raw"`の別名です。`compact`や`full`のような従来の`outputVariant`の値は、引き続き受け付けられ、レガシー出力を使用します。ビジュアルフォームでは、それらをレガシーの別名としてラベル付けしています。
...
- アカウントの過去データ取得には、Search Termとして`from:user since:YYYY-MM-DD until:YYYY-MM-DD`を使用します。
...
- `tweetIds`を渡すと、多数のポストをまとめて検索できます。1つのアカウントのポストを読むには、プロフィールのURLを貼り付けます。
- 多数のクエリを実行する場合は、各結果にその取得元となった検索語をタグ付けするために`includeSearchTerms: true`を設定します。
- `queryType: "Latest + Top"`を設定すると、Xの2つの検索モードを1回の実行で使えます。重複排除と結果の上限は両方のモードにまとめて適用されます。
...
**どのくらい速いですか？** 実行時間は、入力内容、結果件数、Xの可用性によって異なります。

**最新の検索が、Xの最新タブに表示されないポストを返すのはなぜですか？** Xは、一致するポストの一部を最新タブから外しています。このActorはそれらのポストも返します。どのポストもクエリに対する実際のXの検索結果で、各ポストへの課金は1回だけです。

**どの検索演算子が使えますか？** Xの高度な検索は、著者、宛先、メンション、日付、エンゲージメント、メディア、位置情報をサポートしています。
