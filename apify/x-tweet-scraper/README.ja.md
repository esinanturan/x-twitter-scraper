...
`completionReason: "pagination_safety_limit"`は読み取り失敗ではありません。これは、ページネーションが有効な行を保持したまま、設定された安全上限に達したことを意味します。有効な回復用カーソルが残っている限り、最新の検索は空のページを経ても続行されます。トップ検索とアカウント期間の回復は、連続して10ページが空だった場合にチェックポイントを取ることがあります。実行の途中でページネーションの停滞をサービスが報告した場合、実行は31秒待ってから同じページをもう1回リクエストします。その後、新しいポストで続行するか、完了として終了します。2回目の停滞で、検索はチェックポイントを取ります。チェックポイントを取った実行は不完全な抽出をレポートし、再開可能なカーソルを保持します。ページが空になり続けた後でも、最終ページに達すればページネーションは完了します。`failedSubtargets`は`0`のままです。お支払いいただくのは、受理されたデータセット行の分のみです。
...
Actorは、ポストのURLを最大100件までの同時バッチで検索します。部分的に成功したレスポンスでは、未解決のIDのみを1回だけ再チェックします。バッチ出力は一意であり、要求されたIDと一致します。プロフィールのURLは、プロフィールのタイムラインと著者検索を組み合わせます。検索URLはクエリを抽出します。リストのURLは、汎用の`list:`検索ではなく専用のリストパスを使用します。`maxItems`は、貼り付けたすべてのURLをまたいだ結果の上限です。
...
**最新の検索が、Xの最新タブに表示されないポストを返すのはなぜですか？** Xは、一致するポストの一部を公開の最新リストから外し、時間範囲を指定した検索にだけ返します。このActorは最新の検索を並んだ時間スライスとして読み取るため、両方を取得します。1つのクエリで100件のポストを調べたテストでは、83件が他の5つのスクレイパーが返したポストと一致し、17件はXが時間範囲を指定した検索にだけ返したポストでした。17件はすべて同じ時間範囲内にありました。どのポストもあなたのクエリに対するXの実際の検索結果であり、各ポストへの支払いは1回だけです。
