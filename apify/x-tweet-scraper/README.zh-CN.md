...
未指定构建时，Apify 会使用此 Actor 的 `latest` 默认值。控制台运行和标准
API 示例都会继承该默认值。
...
仍会完成分页。`failedSubtargets` 会保持为 `0`。只有被接受的数据集行才会
计费。

Apify 的默认超时为 `0`，因此运行没有时间限制。Actor 会持续运行，直到
达到上限或可用数据耗尽为止。调用方仍可设置有限的 Apify 超时。此时
...
推文 URL 会以最多 100 个为一批并发查询。部分成功的响应会对未解析的
...
每个用户名都会结合游标分页和作者搜索。Actor 会在输出和计费之前移除
重复行。用户名可以带上可选的 `@` 前缀。
...
回复类帖子。计费之前会排除回复行及来自其他作者的对话上下文。

`profileReplies` 遵循 X 的 With Replies 标签页。它返回目标账户自己
发布的主页帖子及回复，会排除来自其他作者的对话上下文。如果只需要
...
**支持哪些搜索操作符？** X 高级搜索支持按作者、接收者、提及、日期、
互动、媒体及位置进行搜索。
