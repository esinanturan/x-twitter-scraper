...
## File media e metadati

- Post, profili, risposte, citazioni e thread in blocco.
- Foto, GIF, qualità MP4 e playlist HLS.
- Metadati, URL diretti e filtri disponibili.
- Archiviazione entro i limiti di esecuzione, link di output e deduplicazione
  pre-fatturazione.
...
Le righe includono disponibilità, metadati e URL di azione. Le righe
archiviate aggiungono `storedMediaUrl` e `downloadStatus: "stored"`.
**Stored Media Files** consente di sfogliare i record archiviati.
...
- La deduplicazione avviene prima della fatturazione. Un addebito per ogni
  riga di dati consegnata.
- La diagnostica è gratuita in `diagnostics`. Nessun costo di avvio, URL,
  file o qualità.

Usa `latest`. Scegli tra 50 task pubblici o 129 operazioni REST. Gli esempi
usano valori campione. I risultati riflettono dati in tempo reale.
...
`pagination_safety_limit`, `reply_reach` e `deadline_reached`. `reply_reach`
significa che X ha fornito solo una parte di un thread di risposte. Un target
mancante entra nell'elenco solo se un'altra causa ha interrotto l'esecuzione.
