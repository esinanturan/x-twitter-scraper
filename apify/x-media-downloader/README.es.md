...
causas son `target_not_found`, `target_failed`, `pagination_safety_limit`,
`reply_reach` y `deadline_reached`. `reply_reach` significa que X entregó solo
una parte de un hilo de respuestas. Un objetivo faltante se suma a la lista solo
si otra causa detuvo la ejecución. La ejecución es `retryable` cuando alguna
causa lo es.
