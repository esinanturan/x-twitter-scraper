...
den umfassendsten X-Daten, und X Media Downloader extrahiert oder speichert
...
den Medien-Tabs von Profilen. Sammle Fotos, Videos, GIFs und Metadaten. Kein
X-API-Schlüssel oder Login erforderlich.

## Mediendateien und Metadaten

- Beiträge, Profile, Antworten, Zitate und Threads in großen Mengen.
- Fotos, GIFs, MP4-Qualitätsstufen und HLS-Playlists.
- Metadaten, direkte URLs und verfügbare Filter.
- Speicherung innerhalb der Run-Limits, Ausgabe-Links und Deduplizierung vor
  der Abrechnung.
...
Datensätze enthalten Verfügbarkeit, Metadaten und Aktions-URLs. Gespeicherte
Datensätze ergänzen `storedMediaUrl` und `downloadStatus: "stored"`.
**Stored Media Files** durchsucht gespeicherte Datensätze.
...
- Die Deduplizierung läuft vor der Abrechnung. Eine Gebühr pro geliefertem
  Datensatz.
- Diagnosen in `diagnostics` sind kostenlos. Keine Start-, URL-, Datei- oder
  Qualitätsgebühr.

Verwende `latest`. Wähle aus 50 öffentlichen Tasks oder 129
REST-Operationen. Beispiele nutzen Beispielwerte. Ergebnisse spiegeln
Live-Daten.
