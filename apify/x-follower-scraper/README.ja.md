...
- 自動カーソルは、1ページあたり最大300件のプロフィールを要求します。
- 古いカーソルは200件のプロフィール上限を維持し、期限切れになると最初から再開します。
- ページログには、ターゲットを重複させることなく `fetchDurationMs`、`processingDurationMs`、`pushDurationMs`、`statusDurationMs`、`fullPageDurationMs` が含まれます。
- チェックポイントは、再起動後も承認済みの行、タイミング、失敗回数を保持します。
...
閲覧者に紐づく状態は、あなたのデータセットではなくXquikの取得用アカウントに属します。フォロー、ブロック、ミュート、DM、通知などの閲覧者フラグは、raw出力を含めて常に除去されます。
...
`failedTargets` は、読み取り失敗の後に停止したターゲットの数をカウントします。承認済みのプロフィールは課金対象のデータ行のままです。これらの実行では `completionReason: "partial_failure"` が使用されます。高速なサーバー側のページネーションも、同じ報告契約に従います。

Apifyのデフォルトのタイムアウトは `0` です。実行に時間制限はありません。Actorは、上限またはソースが尽きるまで、あらゆるライブカーソルをたどり続けます。呼び出し側は、それでも有限のタイムアウトを設定できます。その場合、`completionReason: "deadline_reached"` は、その上限が近いことを意味します。Actorは、チェックポイント、行、レポート、正常終了のために、最後の15秒を確保します。有効なプロフィールは配信され、一度だけ課金されます。未完了のページネーションは再開可能なままです。

独立したターゲットは並行して実行されます。各ターゲットは順序付けられたカーソルページネーションを維持します。データセットへの書き込みは、上限、重複排除、帰属情報、課金をアトミックに保ちます。
...
**速度はどれくらいですか?** 実行時間は、ターゲットの規模、フィルター、上流の可用性によって異なります。深いフィルター付きの実行は、5ページごとにConsoleの進捗をチェックポイントします。これにより、ページ取得の間の非データ通信が減ります。

**実行の結果が`maxItems`より少ないのはなぜですか?** `minFollowers`、`verifiedOnly`、`bioContains` などのフィルターは書き込み前に適用されます。より多くの結果を得るには、フィルターを緩めてください。

**1つのアカウントから何人のフォロワーをスクレイピングできますか?** Xは大規模なアカウントをバッチでページネーションします。より多くのページを取得するには、Apifyの実行時間制限を引き上げてください。`maxItemsPerTarget` は各ターゲットのみを制限します。

**Actorは一時的な失敗を再試行しますか?** はい。タイムアウト、429、5xxのレスポンスに対して、ページごとに最大3回試行します。`Retry-After` が存在する場合はそれに従います。存在しない場合は指数バックオフを使用します。致命的な失敗の場合でも、部分的な結果は保持されます。

**Apifyの実行時間制限が近づくと何が起きますか?** Actorは、それより短い独自の実行期限を追加しません。Apifyの設定された制限を使用し、終了処理のために最後の15秒を確保します。プロフィールをフラッシュし、ページネーションをチェックポイントし、レポートを書き込んで終了します。データセットに承認されなかった行は課金されません。

**中断した箇所から再開できますか?** カーソルを再開するための入力は、まだ公開されていません。同じターゲットを再実行すると、その最初の利用可能なページから開始します。
