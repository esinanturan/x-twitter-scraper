...
elegir la relación que el Actor extrae para cada nombre de usuario.
...
Pega la entrada que ya usas. X Follower Scraper lee los nombres de campo que
usan otros Actors de seguidores de X y los asigna a sus propios campos. Los
nombres canónicos siguen siendo la opción documentada por defecto. Un alias
nunca descarta un campo ni cambia lo que pagas.

| Campo que ya usas                                                                     | X Follower Scraper lo lee como |
| ------------------------------------------------------------------------------------- | ------------------------------ |
...
| `scrapeAllResults`                                                                    | sin límite por objetivo        |

2 nombres significan otra cosa aquí. En algunos Actors, `maxFollowers` y
`maxFollowing` limitan cuántas filas obtiene una ejecución. En X Follower
Scraper filtran perfiles por sus cantidades de seguidores y de cuentas
seguidas. Usa `maxItems` para limitar las filas. X Follower Scraper no tiene
una unidad de página, así que reemplaza `maxPages` por `maxItems`.
