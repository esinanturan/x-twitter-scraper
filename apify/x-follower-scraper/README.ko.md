...
행에는 뷰어 전용 상태가 절대 포함되지 않습니다. 팔로우, 차단, 뮤트, DM, 알림 및 유사한 뷰어 플래그는 raw 출력을 포함해 항상
제거됩니다.
...
기본 Apify 시간 제한은 `0`입니다. 실행에는 시간 제한이 없습니다. Actor는 상한에 도달하거나 프로필이 더 이상 없을 때까지
계속됩니다. 유한한 시간 제한을 직접 설정할 수도 있습니다. 그러면 `completionReason: "deadline_reached"`는 그
제한이 가까워졌다는 뜻입니다. Actor는 제한 전에 프로필 & 보고서를 저장하고 정상적으로 종료합니다. 전달된 프로필은 한 번만 과금됩니다.
...
**속도는 얼마나 빠른가요?** 실행 시간은 대상 크기, 필터, X 가용성에 따라 달라집니다.

**왜 내 실행이 `maxItems`보다 적은 행을 반환하나요?** `minFollowers`,
`verifiedOnly`, `bioContains` 같은 필터는 쓰기 전에 적용됩니다. 더 많은 결과를
얻으려면 필터를 완화하세요.

**하나의 계정에서 얼마나 많은 팔로워를 스크랩할 수 있나요?** X가 해당 계정에 대해 보여 주는 만큼 가져올 수 있습니다. 실행은 상한,
지출 한도 또는 목록의 끝에 도달할 때까지 계속됩니다. `maxItemsPerTarget`은 각 대상만 제한합니다.

**Actor가 일시적인 실패를 재시도하나요?** 예. X의 일시적인 오류에서 자동으로 복구됩니다. 심각한 실패가 발생해도 부분 결과는
유지됩니다.

**Apify 실행 시간 제한 근처에서는 어떻게 되나요?** Actor는 자체적으로 더 짧은 기한을 추가하지 않습니다. 제한 전에 프로필을
저장하고 보고서를 작성한 뒤 종료합니다. 데이터셋에 도달하지 않은 행은 과금되지 않습니다.

**중단한 곳에서 다시 시작할 수 있나요?** 아직은 안 됩니다. 동일한 대상을 다시 실행하면 처음부터 시작합니다.
