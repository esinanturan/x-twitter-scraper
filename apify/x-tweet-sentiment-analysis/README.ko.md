...
Xquik은 가장 완전한 X 데이터를 보유한, 세계에서 가장 빠르고 저렴한 X(Twitter) 스크레이퍼 서비스입니다. X Tweet
Sentiment Analysis는 모든 트윗에 태도, 강도 & 비꼬는 표현 가능성을 추가합니다. 다른 모든 Apify Actor는 필터링이나
...
대상을 제공하면 감정은 대상에 대한 태도를 판단하고 직접 제공한 인용이나 답글
맥락을 사용합니다. 대상이 없으면 게시물의 주된 주제를 판단합니다.
...
수집이 포함됩니다. 분석 허용량은 질문 8개, 질문 정의당 8,000바이트, 트윗당
맥락 12,000바이트입니다. 추출 필터 & 중복 제거는 분석 전에 실행되므로
필터링되어 제외되거나 중복된 행은 분석되거나 과금되지 않습니다. 실패하거나
건너뛴 분석과 진단 행에는 결과 요금이 없습니다. Apify는 플랫폼
...
`sentiment` 분포는 각 태도에 속하는 트윗 수를 보여줍니다.
`engagementShares`는 모든 트윗을 좋아요, 리트윗, 답글 & 인용으로 가중한
같은 분포를 보여줍니다. `top`은 태도별로 가장 많이 참여된 트윗 3개를
나열합니다. 요약은 숫자를 소수점 4자리로 반올림합니다. 빈 실행은 0 카운트 &
...
`previous`에서 `current`로 바뀐 각 감정, 강도 수준, 비꼬기 판단을 나열합니다. 판단은 카테고리, 반올림된 점수 수준, 또는
...
반환합니다. `unclear` 카테고리 & 확률이 모든 언어에서 불확실성을
보여줍니다.
