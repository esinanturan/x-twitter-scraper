...
X가 공개한 아티클만 반환할 수 있습니다. 마크다운은 블록, 굵게, 기울임꼴 범위를
보존합니다. `contents`는 원본 서식을 유지합니다. 링크 메타데이터는 추측하지
않습니다. Apify는 마크다운을 텍스트로 표시합니다. 예시는 샘플 값을 사용합니다.
결과는 실시간 데이터를 반영합니다. `latest`를 사용하세요. URL과 ID는 함께 사용할
수 있습니다.
