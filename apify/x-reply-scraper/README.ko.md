...
- 자동 모드는 불완전한 직접 결과를 대화 검색으로 전환합니다.
- 자동 트윗 답글 페이지는 최대 300개 행을 요청합니다.
- 4가지 전략이 직접 답글, 검색, 스레드 맥락을 다룹니다.
- 대량 입력은 게시물 URL, 트윗 ID, 프로필, 사용자 이름을 받아들입니다.
- 프로필 대상은 둘 다 해당할 때 타임라인과 작성자 검색을 결합합니다.
...
- 페이지 & 대상 로그에는 입력을 반복하지 않으면서 `fetchDurationMs`,
  `processingDurationMs`, `pushDurationMs`, `statusDurationMs`,
  `fullPageDurationMs`, `fullTargetDurationMs`가 포함됩니다.
- 체크포인트는 재시작 후에도 승인된 답글, 타이밍, 실패를 보존합니다.
...
초기 폼은 검증된 공개 대화를 대상으로 합니다. 최대 10페이지에 걸쳐 최대 25개의
전체 플랫 행을 반환합니다. 자동 모드는 기본적으로 전체 대화를 검색합니다. 중복
제거와 소스 귀속은 계속 켜져 있습니다.
...
| `usernames`   | 작성자 검색이 포함된 프로필 타임라인         |
| `startCursor` | 저장된 소스 커서로 대상 1개 재개             |
...
프로필 대상은 커서 페이지네이션과 작성자 검색을 결합합니다. Actor는 출력 및
과금 전에 중복 행을 제거합니다. 저장된 레거시 커서는 표준 페이지네이션을
유지합니다.
...
대부분의 작업에는 `collectionStrategy: "auto"`를 사용하세요. 전체 또는 중첩
...
X의 답글 타임라인을 강제로 사용하려면 `collectionStrategy: "replies"`를
사용하세요. 소스 순서를 유지하며 커서를 지원합니다.

### 대화 검색

넓은 대화 커버리지를 위해 `collectionStrategy: "conversationSearch"`를
사용하세요. Actor는 `conversation_id:<Tweet ID>`로 검색합니다.
...
Xquik은 전달된 데이터 행당 1회 과금합니다. 진단 정보는 `diagnostics`에서
무료입니다. 시작, URL, 쿼리, 페이지네이션, 필터, 프록시 요금은 없습니다.

기본 Apify 시간 제한은 `0`이므로 실행에는 시간 제한이 없습니다. Actor는 상한에
도달하거나 적격 데이터를 모두 쓸 때까지 계속 실행합니다. 호출자는 여전히 유한한
Apify 시간 제한을 설정할 수 있습니다. 그러면 `completionReason: "deadline_reached"`는
해당 설정된 제한이 임박했음을 의미합니다. Actor는 체크포인트, 행, 보고서, 정상
종료를 위해 마지막 15초를 남겨둡니다. 이미 수집된 답글은 계속 전달되며 한 번만
과금됩니다. 마치지 못한 페이지네이션은 재개할 수 있습니다.
...
Actor는 보호된 계정을 우회하지 않습니다. 고객의 X 비밀번호, 세션 쿠키, 인증
토큰을 요청하지 않습니다.
