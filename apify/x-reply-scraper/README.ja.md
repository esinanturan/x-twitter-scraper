...
- `collectionStrategy`は、用途に応じた4つのモードを提供します。
...
- 実行ログには、ページごと・ターゲットごとの所要時間が`fetchDurationMs`、`processingDurationMs`、`pushDurationMs`、`statusDurationMs`、`fullPageDurationMs`、`fullTargetDurationMs`として表示されます。
- Apifyが実行を再起動しても、配信済みのリプライと進捗は保持されます。

### 常に最新のビルドを使用する

公開済みのすべての修正を受け取るには、実行のたびに `latest` を選択してください。

ビルドを指定しない場合、ApifyはこのActorの `latest` デフォルトを使用します。Consoleでの実行や標準的なAPIの例は、そのデフォルトを引き継ぎます。
...
初期フォームは、確認済みの公開会話を対象にしています。最大25件の完全なフラット行を返します。自動モードは、デフォルトで会話全体を検索します。重複排除とソースの帰属は有効のままです。
...
| `tweetIds`    | 数値の投稿ID                                       |
| `usernames`   | プロフィールのリプライタイムライン                 |
| `startCursor` | 保存済みのカーソルから1つのターゲットを再開         |
...
Actorは、出力と課金の前に重複行を除去します。

## カバレッジ戦略

### 自動での完全収集

ほとんどのジョブには`collectionStrategy: "auto"`を使用してください。指定した範囲で取得できるすべてのリプライを収集します。範囲、深さ、並び順、著者の設定は、上限より前に適用されます。ルート以外の対象の下にあるリプライも含まれます。Xがスレッドの一部を非表示にしている場合、ステータスにはXが非表示にしているリプライの数が示されます。その他の`collectionStrategy`の値がモードを切り替えることはありません。

診断情報のカバレッジの数値は、Xにそれ以上リプライがないことを証明するものではありません。上限、データの欠落、エラーによって、実行が未完了のままになることがあります。

### 直接のリプライ

Xの表示順で直接のリプライを取得するには、`collectionStrategy: "replies"`を使用します。保存済みのカーソルに対応しています。

### 会話検索

会話を幅広くカバーするには、`collectionStrategy: "conversationSearch"`を使用します。
...
Xquikは、配信されたデータ行1件ごとに1回課金します。`diagnostics`の診断情報は無料です。開始、URL、クエリ、ページネーション、フィルターの料金はかかりません。

デフォルトのApifyタイムアウトは`0`なので、実行に時間制限はありません。Actorは、上限に達するか、対象データを使い尽くすまで継続します。有限のApifyタイムアウトを設定することもできます。その場合、`completionReason: "deadline_reached"`は、その上限が近づいていることを意味します。Actorは上限の前にリプライとレポートを保存し、正常に終了します。配信済みのリプライは1回だけ課金されます。未完了のターゲットは後から再開できます。
...
Actorは非公開アカウントの保護を回避しません。Xのパスワード、Cookie、トークンを求めることもありません。
