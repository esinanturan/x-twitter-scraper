...
- 运行日志会在 `fetchDurationMs`、`processingDurationMs`、`pushDurationMs`、`statusDurationMs`、`fullPageDurationMs` 和 `fullTargetDurationMs` 中显示每页和每个目标的耗时。
- 如果 Apify 重启运行，已交付的回复和进度都会保留。
...
初始表单以一个已验证的公开对话为目标，最多返回 25 行完整的扁平数据。自动模式默认搜索整个对话。去重和来源归属保持开启。
...
Actor 会在输出和计费之前移除重复行。

## 覆盖策略

### 自动完成

大多数任务请使用 `collectionStrategy: "auto"`。它会收集在你的范围内能获取到的所有回复。范围、深度、排序和作者控制会在你的上限之前生效。非根目标下的回复也会包含在内。当 X 隐藏了帖子串的一部分时，状态消息会说明 X 隐藏了多少条回复。其他 `collectionStrategy` 值永远不会切换模式。

诊断中的覆盖率数值并不能证明 X 已没有更多回复。上限、缺失数据或错误都可能让运行保持不完整。

### 直接回复

使用 `collectionStrategy: "replies"` 可按 X 自身的顺序获取直接回复。它支持已保存的游标。

### 对话搜索

使用 `collectionStrategy: "conversationSearch"` 可获得更广的对话覆盖。
...
Xquik 对每一行已交付的数据行收取一次费用。`diagnostics` 中的诊断记录免费。不收取启动、URL、查询、分页或过滤费用。

Apify 的默认超时为 `0`，因此运行没有时间限制。Actor 会持续运行，直到达到上限或用完可用数据为止。你仍可设置有限的 Apify 超时。此时 `completionReason: "deadline_reached"` 表示该限制即将到达。Actor 会在到达限制前保存回复和报告，然后正常退出。已交付的回复只计费一次。未完成的目标仍可继续。
...
Actor 不会绕过受保护的账户，也从不索取你的 X 密码、Cookie 或令牌。
