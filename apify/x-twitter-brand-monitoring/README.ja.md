...
Xquikは世界最速かつ最安値のX(Twitter)スクレイパーサービスで、最も網羅的なXデータを提供します。X (Twitter) Brand Monitoringは、関連性、感情、顧客体験の回答であなたのブランド言及を追跡します。他のApify Actorはすべて、フィルタリングや重複排除の前に課金します。Xquikは、配信済みでユニークかつフィルタ条件に一致する結果にのみ課金します。AI の費用はツイート単価に含まれています。AI プロバイダーへの支払いも、トークンの購入も、キーの用意も不要です。

X(Twitter)上でのブランド言及を監視し、実行間の感情の変化を追跡します。**X (Twitter) Brand Monitoring with AI Analysis**は、一致するすべてのポストを収集します。各ポストについて、関連性、感情、顧客体験の質問にAIで回答します。その回答を以前のデータセットと比較するので、何が変化したかがわかります。すべての行が元のポストデータを保持するため、エクスポート、レビュー、追加分析のために再度スクレイピングする必要はありません。

ブランド、製品ライン、キャンペーンについて、苦情、称賛、購入に関する質問を監視できます。実際のポストをもとにサポートチームやマーケティングチームに報告できます。顧客があなたについてどのように語っているかの履歴を、実行ごとに保持できます。

- **元のポストのすべてのフィールド。** テキスト、著者、各種カウント、メディア、リンク、引用ポストとリプライ対象のポストが回答とともに残ります。
- **型付きの回答。** 各行には、関連性の確率、確率付きの感情カテゴリ、顧客体験のカテゴリがあります。
- **変化の追跡。** 実行は判定単位で比較するため、確率のわずかな変動は変化として数えません。
- **フィルタ優先の課金。** 分析に成功した、一意でフィルタ条件に一致するポストにのみ料金がかかります。
...
正常に分析されたポスト1件につき$0.0003から、開始料金はかかりません。料金には収集が含まれます。分析上限は質問8個、質問定義1個あたり8,000バイト、ポスト1件あたりのコンテキスト12,000バイトです。抽出フィルタと重複排除は分析より前に実行されるため、フィルタで除外された行や重複行に料金を支払うことはありません。失敗した分析、スキップされた分析、診断情報の行には結果料金は発生しません。Apifyは、計算、ストレージ、転送のプラットフォーム利用料を、あなたのプランの料金で別途請求します。Pricingタブに表示されます。
...
- `targets`は、ブランドまたはエイリアスごとの言及数、シェア・オブ・ボイス、エンゲージメントをレポートします。
- 各`targets`エントリには`top`があり、回答カテゴリごとに最もエンゲージメントの高い言及を3件示します。最も強いネガティブおよびポジティブな言及のアラートに使用してください。
- 各`targets`エントリには`choices`があり、そのブランドに言及したポストの中での回答の内訳を示します。
- `sentiment`ブロックは、`top`の下に最もエンゲージメントの高いポジティブおよびネガティブな言及を3件ずつ一覧表示します。
- `relevance`は、そのブランドに関する言及の数を数えます。
- `monitor.changedRows`は、ベースラインから判定が変化したポストを一覧表示します。Webhookやアラートに送信してください。
- `monitor.baselineDatasetId`が設定されている場合、`monitor`ブロックが比較ステータスを数え、変更のあった行を最大50件まで一覧表示します。
- 各行は、リンク先のホスト名である`sourceDomains`を一覧表示します。

サマリーは数値を小数点以下4桁に丸めます。空の実行は、カウントをゼロ、平均を`null`としてレポートします。
...
はい。カスタムの`analysis.questions`はデフォルトを置き換えます。`choice`、`score`、`probability`の質問を1～8個送信してください。choiceの質問は2～255個のカテゴリを受け付けます。scoreの質問は少なくとも2段階の順序付きレベルを使用します。比較したい実行間では、同じ質問を維持してください。
...
抽出はXが提供するすべての言語に対応しています。分析はまず英語の顧客シナリオで検証しています。その他のサポート対象言語でも同じ構造の回答を返します。`unclear`のカテゴリと確率が、すべての言語で不確実性を示します。
