...
답변으로 브랜드 언급을 추적합니다. 다른 모든 Apify Actor는 필터링이나 중복 제거
...
Actor는 답변을 판단 기준으로 비교합니다. `choice` 답변은 카테고리로 비교합니다.
`score` 답변은 가장 가까운 수준으로 비교합니다. `probability` 답변은 0.5 기준의
예/아니오 판단으로 비교합니다. 판단은 세 가지 경우에 변경된 것으로 집계됩니다.
이전 카테고리의 확률이 0.4 아래로 떨어집니다. 점수가 최소 0.6 수준 이동합니다.
예/아니오 확률이 임계값에서 최소 0.1 벗어납니다. 실행 간의 근소한 차이는
`unchanged`로 남고, 동일한 판단을 유지하는 변동도 마찬가지입니다. 실행 간 모델
편차가 보고서를 채우지 않습니다. `changes`는 변경된 각 질문을 `previous` &
`current` 판단과 함께 나열합니다. 변화는 모델 편차, 새로운 맥락, 편집된 소스
데이터에서 올 수 있습니다. 변화는 사실이 바뀌었다는 증거가 아니며, 트윗이 없다고
해서 삭제되었다는 증거도 아닙니다.
...
AI 비용은 트윗당 가격에 포함되어 있습니다. AI 제공업체에 비용을 내거나 토큰을
구매하거나 키를 준비할 필요가 없습니다.
