...
den umfassendsten X-Daten, und X (Twitter) Stock & Crypto AI Trading Signals
wandelt Tweets in bullische, bärische, neutrale oder gemischte Haltungen pro
Ticker & Coin um. Jeder andere Apify Actor berechnet, bevor gefiltert oder
...
& behalte die ursprünglichen Tweet-Daten. **X (Twitter) Stock & Crypto AI Trading Signals** sammelt Beiträge zu deinen Tickern oder Assets, ergänzt
dann jeden Beitrag um eine KI-gestützte Haltung, einen Inhaltstyp, eine
Überzeugungsstufe & eine Asset-Relevanz. Trenne feste Ansagen von
abgesicherten Bemerkungen, Analyse von Werbung, & Beiträge über dein Asset
von unabhängigen Nutzungen seines Namens.

- **Haltung pro Beitrag**: bullisch, bärisch, neutral, gemischt oder
  unklar.
...
`cashtags` zählt die Haltung pro Cashtag wie `$NVDA`, sodass die
bullische Quote pro Asset aus `choices.stance` stammt. Der
`stance`-Block ergänzt die interaktionsgewichtete Aufteilung & die am
stärksten interagierten bullischen & bärischen Beiträge; `conviction`
meldet den Mittelwert & den interaktionsgewichteten Mittelwert. Zahlen
sind auf 4 Nachkommastellen gerundet; leere Runs melden Nullwerte &
`null`-Mittelwerte. Jeder `cashtags`-Eintrag ergänzt `signal`: Anzahl
bullisch, Anzahl bärisch & einen Score von -1 bis 1, berechnet als
(bullisch - bärisch) / Datensätze, und `monitor.changedRows` listet
Tweets, deren Haltung sich seit der Baseline geändert hat. Jeder
Datensatz listet außerdem `sourceDomains`, die Hostnamen, auf die er
verlinkt, und der `monitor`-Block der Zusammenfassung zählt
...
jede Haltungs-, Inhaltstyp- oder Überzeugungsstufen-Entscheidung listet,
die sich von `previous` zu `current` geändert hat. Entscheidungen werden
nach Kategorie, gerundeter Score-Stufe oder Ja/Nein bei 0,5 verglichen,
und eine Entscheidung zählt nur als geändert, wenn sich die Antwort
deutlich verschiebt: die frühere Kategorie fällt unter eine
Wahrscheinlichkeit von 0,4, ein Score bewegt sich um mindestens 0,6
Stufen, oder eine Ja/Nein-Wahrscheinlichkeit landet mindestens 0,1 vom
Schwellenwert entfernt. Fast unentschiedenes Rauschen zwischen Runs
