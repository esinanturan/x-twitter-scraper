...
Stock & Crypto AI Trading Signals는 트윗을 티커나 코인별 강세, 약세, 중립 또는 혼조 입장으로 바꿉니다. 다른 모든
Apify Actor는 필터링이나 중복 제거 전에 요금을 부과합니다. Xquik은 전달되고, 고유하며, 필터에 맞는 결과에만 요금을 부과합니다.
AI 비용은 트윗당 가격에 포함되어 있습니다. AI 계정, 토큰, 키가 필요하지 않습니다.

X(Twitter)의 주식, 암호화폐 & 트레이딩 게시물 뒤에 담긴 입장을 읽으면서 원본
트윗 데이터는 그대로 유지하세요. **X (Twitter) Stock & Crypto AI Trading
Signals**는 여러분의 티커나 자산에 관한 게시물을 수집합니다. 그런 다음 모든
게시물에 AI 기반 입장, 콘텐츠 유형, 확신 수준 & 자산 관련성을 추가합니다. 확고한
주장을 헤지된 발언에서 분리하고, 분석을 홍보에서 구분하며, 자산에 관한 게시물을
그 이름을 무관하게 사용한 게시물과 구분하세요.

- **게시물별 입장.** 각 게시물은 강세, 약세, 중립, 혼조 또는 불명확입니다.
- **콘텐츠 유형**은 분석, 뉴스, 트레이딩 아이디어, 홍보, 유머, 질문을
  구분합니다.
- **확신도**는 확고한 주장 & 포지션을 헤지된 발언에서 구분합니다.
- **관련성**은 티커나 회사명을 무관하게 사용한 것을 걸러냅니다.
- 트윗이 노출하는 모든 필드가 담긴 **완전한 소스 레코드**.

## X에서 시장 감정을 분석하는 방법

1. `$NVDA lang:en -filter:retweets` 같은 검색어, 캐시태그 쿼리, 프로필
   핸들, 트윗 ID를 추가하세요.
2. `maxItems`와 날짜 경계나 최소 좋아요 같은 추출 필터를 설정하세요.
3. 자산 이름, 티커 & 별칭을 `analysis.targets`에 넣고 `analysis.context`에서
   자산을 설명하세요.
...
| 콘텐츠   | 분석, 뉴스, 트레이딩, 홍보, 유머, 질문 또는 불명확                 |
| 확신도   | 0 헤지된 발언, 1 명시된 견해, 2 확고한 주장이나 포지션             |
| 관련성   | 게시물이 여러분의 대상을 자산으로 다룰 확률                        |
...
행을 집계하고, 참여도를 합산하며, 모든 질문을 요약합니다. `cashtags`는 `$NVDA`
같은 캐시태그별 입장을 집계하므로 자산별 강세 비율은 `choices.stance`에서 확인할
수 있습니다. `stance` 블록은 참여도 가중 분포와 가장 많이 참여된 강세 & 약세
게시물을 추가합니다. `conviction`은 평균 & 참여도 가중 평균을 보고합니다. 요약은
숫자를 소수점 4자리로 반올림합니다. 빈 실행은 0 카운트 & `null` 평균을
보고합니다. 각 `cashtags` 항목은 강세 수, 약세 수 & -1에서 1 사이 점수가 담긴
`signal`을 추가합니다. 점수는 (강세 - 약세) / 행 수입니다.
`monitor.changedRows`는 기준선 이후 입장이 바뀐 트윗을 나열합니다. 모든 행은
링크된 호스트 이름인 `sourceDomains`를 나열합니다. `monitor.baselineDatasetId`가
설정된 경우 요약의 `monitor` 블록은 비교 상태를 집계하고 변경된 행을 최대
50개까지 나열합니다.
...
`previous`에서 `current`로 바뀐 각 입장, 콘텐츠 유형, 확신 수준을 나열합니다. 판단은 카테고리, 반올림된 점수 수준, 또는
0.5 기준의 예/아니오로 비교됩니다. 판단은 분명하게 바뀐 경우에만 변경된 것으로 집계됩니다. 실행 간의 근소한 흔들림은 변경되지 않은 것으로
...
### 하나의 실행에서 여러 티커를 추적할 수 있나요?

예. 모든 자산을 티커 & 별칭과 함께 `analysis.targets`에 나열하고 검색어를
결합하세요. 관련성 답변은 어떤 게시물이 여러분의 대상을 자산으로 다루는지
알려줍니다.
