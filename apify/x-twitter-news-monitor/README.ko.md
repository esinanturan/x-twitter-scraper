...
표기 & 관련성으로 정렬합니다. 다른 모든 Apify Actor는 필터링이나 중복 제거
전에 요금을 부과합니다. Xquik은 전달되고, 고유하며, 필터에 맞는 결과에만
요금을 부과합니다.
...
출처라는 뜻은 아니며, 출처 표기는 게시물이 제시하는 내용을 설명합니다.
...
사용 가능한 확률이 포함됩니다. 게시물이 X 아티클을 링크할 때 분석은 해당
아티클의 제목, 미리보기, 텍스트 블록을 맥락으로 가져오며,
`analysis.contextAvailability.article`은 `text_blocks`,
`summary`(제목 & 미리보기만) 또는 `not_supplied`를 보고합니다. 실패하거나
건너뛴 분석은 수집된 트윗을 빈 답변 목록과 `reason`과 함께 유지합니다.
키-값 저장소의 무료 진단은 잘못된 입력, 누락된 결과, 중단된 수집을
설명하며, 실행 보고서는 수집된 행, 과금된 분석, 보류 중인 요금을
구분합니다.
...
`format` 분포는 보도를 논평, 추측, 홍보, 풍자와 구분합니다. `attribution`은
명시된 출처, 링크된 출처, 직접 목격, 출처 없음을 집계하며, `relevance`는
각 대상에 관한 게시물 수를 집계하고, `targets`는 대상별 언급 수를
제공하며 각 대상의 `top`은 답변 카테고리별로 가장 많이 참여된 게시물을
나열합니다. 숫자는 소수점 4자리로 반올림되며, 빈 실행은 0 카운트와
`null` 평균을 보고합니다. `sourceDomains`는 실행 전체에서 링크된 도메인을
집계하며, 각 `targets` 항목에는 해당 대상에 관한 게시물의 형식 & 출처
표기 분포인 `choices`가 있고, `monitor.changedRows`는 기준선 이후 판단이
바뀐 게시물을 나열합니다. 모든 행에는 링크된 호스트 이름인
...
되며, `changes`는 `previous`에서 `current`로 바뀐 각 형식, 출처 표기,
관련성 판단을 나열합니다. 판단은 카테고리, 반올림된 점수 수준, 또는 0.5
