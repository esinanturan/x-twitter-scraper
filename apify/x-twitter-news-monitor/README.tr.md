...
scraper hizmetidir. X (Twitter) News Monitor, haber gönderilerini biçim, kaynak
atfı ve ilgiye göre sıralar. Diğer tüm Apify Actor'ları filtreleme veya
...
fiyata dahil. Yapay zekâ sağlayıcısına ödeme yapmazsın, token almazsın & anahtar
getirmezsin.

X (Twitter)'daki haber gönderilerini ne olduklarına göre sırala ve orijinal
tweet verisini sakla. **X (Twitter) News Monitor with AI Analysis**, konularınla
ilgili gönderileri toplar, ardından her gönderiye yapay zeka destekli bir biçim,
kaynak atfı ve ilgi yanıtı ekler. Haberciliği yorum & spekülasyondan ayır. Bir
gönderinin kaynak adlandırıp adlandırmadığını veya kaynağa bağlantı verip
vermediğini gör. Yalnızca izlediğin kuruluşlar, kişiler veya konularla ilgili
gönderileri sakla.
...
Sınıflandırma gerçekleri doğrulamaz. Adlandırılmış bir kaynak, güvenilir bir
kaynak değildir. Atıf, gönderinin ne sunduğunu açıklar.
...
bayt ve tweet başına 12.000 bayt bağlamdır. Çıkarma filtreleri ve tekilleştirme
...
Her sonuç `tweet` ve `analysis` içerir. Yanıtlar türleri, soru sürümlerini ve
mevcut olasılıkları içerir. Bir gönderi bir X Makalesine bağlantı verdiğinde
analiz, o makalenin başlığını, önizlemesini ve metin bloklarını bağlam olarak
alır. `analysis.contextAvailability.article`, `text_blocks`, `summary` veya
`not_supplied` bildirir. `summary`, yalnızca başlık & önizleme demektir.
Başarısız veya atlanan bir analiz, boş bir yanıt listesi ve bir `reason` ile
toplanan tweet'i korur. Anahtar-değer deposundaki ücretsiz tanılamalar, geçersiz
girdileri, eksik sonuçları ve kesintiye uğrayan toplamayı açıklar, çalıştırma
raporu ise toplanan satırları, ücretlendirilen analizleri ve bekleyen ücretleri
ayırır.
...
- `format` bölünmesi, haberciliği yorumdan, spekülasyondan, tanıtımdan &
  hicivden ayırır.
- `attribution`, adlandırılmış, bağlantılı, ilk elden & olmayan kaynakları
  sayar.
- `relevance`, her hedefle ilgili gönderileri sayar.
- `targets`, hedef başına bahsedilmeleri verir. Her hedefin `top`'u, yanıt
  kategorisi başına en çok etkileşim alan gönderilerini listeler.
- Her `targets` girişinde `choices` bulunur. Bu, o hedefle ilgili gönderilerin
  biçim & atıf bölünmesidir.
- `sourceDomains`, çalıştırma genelindeki bağlantılı alan adlarını sayar.
- `monitor.changedRows`, kararları temel değerden bu yana değişen gönderileri
  listeler.

Her satır, bağlantı verdiği ana bilgisayar adlarını `sourceDomains`'te &
metninde bulunan `$NVDA` gibi `cashtags`'i listeler. `monitor.baselineDatasetId`
ayarlıysa özetin `monitor` bloğu karşılaştırma durumlarını sayar & 50'ye kadar
...
bulunmayan tweet'ler için `new_to_baseline` & önceki çalıştırmada bulunan
tweet'ler için `unchanged` veya `changed` olur. `changes`, `previous`'tan
`current`'a geçen her biçim, atıf veya ilgi kararını listeler. Actor kararları
...
olasılığı eşikten en az 0,1 uzağa düşer. Çalıştırmalar arasındaki yakın-berabere
