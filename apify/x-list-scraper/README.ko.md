...
스크레이퍼 서비스입니다. X List Scraper는 리스트 게시물, 멤버 & 팔로워를
수집합니다. 다른 모든 Apify Actor는 필터링이나 중복 제거 전에 요금을
부과합니다. Xquik은 전달되고, 고유하며, 필터에 맞는 결과에만 요금을 부과합니다.

리스트 URL이나 숫자 ID에서 X 리스트 게시물, 멤버, 팔로워를 스크랩하세요. 여러
리스트와 리소스를 함께 처리할 수 있습니다. X API 키나 로그인이 필요 없습니다.

## 리스트 데이터 & 필터

- 텍스트, 작성자, 지표, 미디어, 타임스탬프가 포함된 리스트 게시물.
- 리스트 멤버 & 리스트 팔로워.
- 게시물 타임라인에 대한 선택적 답글 포함.
- 날짜, 유닉스 시간, 언어, 미디어, 참여, 인증, 프로필 필터.
- 실행당 여러 리스트 & 리소스 유형.
- 전체 & 리소스별 상한.
- 마이그레이션 후 페이지네이션이 재개됩니다. 중복 행은 제거됩니다.
...
행은 `resultType`으로 `listTweet`, `listMember`, `listFollower` 중 하나를
사용하며 `sourceTarget`에 입력 리스트 ID를 보존합니다. 행 본문은 안정적인
Xquik 트윗이나 프로필 응답 형식을 사용합니다.
...
독립적인 리스트 리소스는 동시에 실행됩니다. 각 커서 계열은 순서가 유지됩니다.
