...
- Filtere nach Zielgruppe, Beiträgen, Kontoalter, Verifizierung, Website,
  Standort, Bio & Nutzername.
- Setze eine Suche von einem gespeicherten Cursor fort. Migrationen erhalten
  akzeptierte Arbeit.
...
Jeder Datensatz enthält ein Profil und die zugehörige Suche in
`sourceTarget`.

## Preise

Jeder Plan kostet **$0.00015 pro geliefertem Profil**. Apify berechnet die
Plattformnutzung separat.

- Eine Gebühr pro geliefertem Datensatz. Diagnosen in `diagnostics` sind
  kostenlos.
- Keine Start-, Such- oder Seitengebühr. Filter und Deduplizierung laufen vor
  der Abrechnung.

Wähle aus 50 öffentlichen Tasks. 129 REST-Operationen nutzen Beispielwerte &
Live-Daten.
