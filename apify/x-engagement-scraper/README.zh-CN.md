...
当 X 仍显示该转发时，Actor 会找到转发时间。较早、已删除或不可用的转发会让时间戳保持为 `null`。该主页仍会保留在输出中。`null` 值并不能证明某个账户从未转发过该帖子。

此选项会让运行变慢。只需要主页结果时，请保持关闭。主页的 `createdAt` 仍然是账户创建日期。推文行包含转发事件时会带有 `retweetedAt`。原帖日期和抓取时间永远不会替代转发时间。结果价格和按已交付行计费的方式保持不变。
...
一次运行可以读取多个帖子和多种互动类型。 Apify 重启运行后，已交付的行和进度都会保留。Actor 不会自行添加时间限制。
