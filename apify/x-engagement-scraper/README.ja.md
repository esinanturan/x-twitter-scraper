...
各照会は、リポストしたユーザーの最新の利用可能なプロフィールページを確認します。アカウントとソース投稿を実際のリポスト記録と照合します。古い、削除済み、または利用できないリポスト記録がある場合、タイムスタンプは `null` になることがあります。タイムスタンプの照会に失敗した場合も `null` のままになります。そのプロフィールは出力に残ります。この照会は、あるアカウントが投稿を一度もリポストしていないことを証明するものではありません。
