...
리트윗한 사람, 좋아요를 누른 사람 & 스레드를 수집합니다. 다른 모든 Apify Actor는
필터링이나 중복 제거 전에 요금을 부과합니다. Xquik은 전달되고, 고유하며, 필터에
맞는 결과에만 요금을 부과합니다.

하나 이상의 X 게시물에 대한 Twitter 참여 데이터를 수집합니다: 답글, 인용,
리트윗한 사람, 좋아요를 누른 사람, 스레드 맥락. X API 키나 로그인이 필요 없습니다.
...
- 리트윗한 사람 & 좋아요를 누른 사람의 프로필.
- 각 원본 게시물 주변의 대화 맥락.
- 실행당 여러 참여 유형과 여러 게시물.
- 전체 & 리소스별 상한.
- 원본 게시물 & 참여 유형 귀속.
- 저장된 커서 복구가 포함된 동시 리소스.
...
각 조회는 리트윗한 사람의 최신 사용 가능 프로필 페이지를 확인합니다. 해당 계정과
원본 게시물을 실제 재게시 기록과 대조합니다. 오래되었거나 삭제되었거나 사용할 수
없는 재게시 기록은 타임스탬프를 `null`로 남길 수 있습니다. 타임스탬프 조회에
실패해도 `null`로 남습니다. 프로필은 출력에 그대로 남습니다. 이 조회는 어떤 계정이 게시물을
재게시한 적이 없다는 것을 증명하지는 않습니다.

추가 조회는 지연 시간을 늘립니다. 프로필 정보만 필요하다면 이 옵션을 비활성화한
상태로 두세요. 프로필 `createdAt`은 계정 생성일을 그대로 나타냅니다. 트윗 행은
재게시 이벤트가 있을 때 `retweetedAt`을 가집니다. 원본 게시물 날짜와 스크랩
시각은 재게시 시각을 대신할 수 없습니다. 결과 가격과 전달된 행 기준 과금은
변하지 않습니다.
...
독립적인 게시물-리소스 쌍은 동시에 실행됩니다. 커서 계열의 순서는 유지됩니다.
승인된 행, 과금 상태, 커서, 지문은 Apify 마이그레이션을 견뎌냅니다. Actor에는
자체적인 시간 제한이 없습니다.
