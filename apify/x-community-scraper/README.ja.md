...
Xquikは、最も完全なXデータを備えた、世界最速かつ最安のX(Twitter)スクレイパーサービスです。X Community Scraperは、コミュニティ情報、投稿、検索、メンバー、モデレーターを収集します。他のApify Actorはすべて、フィルタリングや重複排除の前に課金します。Xquikは、配信済みでユニークかつフィルター条件に一致した結果にのみ課金します。

コミュニティのURLまたはIDから、Twitterコミュニティの情報、投稿、キーワード一致、メンバー、モデレーターを収集します。1回の実行でリソースとコミュニティを組み合わせられます。X APIキーやログインは不要です。

## コミュニティのデータとフィルター

- Xが公開している場合のコミュニティのメタデータとルール。
- 最新のコミュニティ投稿。
- 「最新」または「トップ」の並び順によるキーワード検索。
- コミュニティのメンバーとモデレーター。
- 投稿、メンバー、モデレーターのフィルターは課金前に実行されます。
- 1回の実行で複数のコミュニティとリソースを指定可能。
- リソースごとおよび全体の上限。
- 保存されたカーソルによる復旧を伴う並行読み取り。
...
キーワード検索を行うには、`resources` に `"search"` を含め、`query` を指定してください。

## 出力

各行は `resultType` として `community`、`communityTweet`、`communityMember`、
`communityModerator` のいずれかを使用します。すべての行には、入力コミュニティを示す `sourceTarget` が含まれます。Actorはソースのフィールドを保持し、値を創作しません。
...
- 開始料金、コミュニティ料金、リソース料金、クエリ料金はかかりません。
- 重複排除は課金前に実行されます。
...
独立したコミュニティリソースは並行して実行されます。ページネーションは各カーソル系列の中で順序が保たれます。承認された行、課金状態、カーソル、出力フィンガープリントは、Apifyのマイグレーションを経ても保持されます。Actorには自己設定のタイムアウトはありません。

ActorはXが公開している公開コミュニティのみを返します。利用可能なフィールドはコミュニティによって異なります。
