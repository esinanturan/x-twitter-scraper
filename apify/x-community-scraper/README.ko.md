...
스크레이퍼 서비스입니다. X Community Scraper는 커뮤니티 정보, 게시물, 검색,
멤버 & 모더레이터를 수집합니다. 다른 모든 Apify Actor는 필터링이나 중복 제거
전에 요금을 부과합니다. Xquik은 전달되고, 고유하며, 필터에 맞는 결과에만 요금을
부과합니다.

커뮤니티 URL이나 ID에서 Twitter 커뮤니티 정보, 게시물, 키워드 검색 결과, 멤버,
모더레이터를 수집합니다. 하나의 실행에서 리소스와 커뮤니티를 함께 사용할 수
있습니다. X API 키나 로그인이 필요 없습니다.

## 커뮤니티 데이터 & 필터

- X가 공개하는 경우 커뮤니티 메타데이터 & 규칙.
- 최신 커뮤니티 게시물.
- 최신순 또는 인기순 정렬을 지원하는 키워드 검색.
- 커뮤니티 멤버 & 모더레이터.
- 게시물, 멤버, 모더레이터 필터는 과금 전에 실행됩니다.
- 실행당 여러 커뮤니티 & 리소스.
- 리소스별 & 전체 상한.
- 저장된 커서 복구가 포함된 동시 읽기.
...
키워드 검색을 하려면 `resources`에 `"search"`를 포함하고 `query`를 지정하세요.

## 출력

행은 `resultType`으로 `community`, `communityTweet`, `communityMember`,
`communityModerator` 중 하나를 사용합니다. 모든 행에는 입력 커뮤니티를 나타내는
`sourceTarget`이 포함됩니다. Actor는 원본 필드를 보존하며 값을 지어내지
않습니다.
...
독립적인 커뮤니티 리소스는 동시에 실행됩니다. 각 커서 계열 내에서 페이지네이션
순서는 유지됩니다. 승인된 행, 과금 상태, 커서, 출력 지문은 Apify 마이그레이션을
견뎌냅니다. Actor에는 자체적인 시간 제한이 없습니다.

Actor는 X가 공개한 커뮤니티만 반환합니다. 사용 가능한 필드는 커뮤니티마다
다릅니다.
