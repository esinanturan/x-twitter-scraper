...
管理员。可在一次运行中混合多种资源与多个 Community。无需 X API 密钥或登录。

## Community 数据与过滤条件

- X 公开时提供 Community 元数据与规则。
- 最新 Community 帖子。
- 支持按“最新”或“热门”排序的关键词搜索。
- Community 成员与管理员。
- 帖子、成员与管理员过滤在计费前进行。
...
一次运行可以读取多个 Community 及其资源。 Apify 重启运行后，已交付的行和进度都会保留。Actor 不会自行添加时间限制。
