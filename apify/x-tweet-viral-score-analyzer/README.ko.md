...
- 고정되고 버전이 관리되는 규칙으로 계산한 **게시물별 Viral Score**.
...
후킹, 명확성, 얻는 가치, 예상 반응은 점수를 높입니다. 흔한 기계식 문구처럼 읽히는 표현은 점수를 낮춥니다.

하드 스톱은 스팸, 분노 유발 글, 흔한 기계식 문구로 보이는 게시물의 점수에 상한을 둡니다. 점수는 0에서 100 사이의 정수입니다.
...
`viral.weights`는 `viral_lite:1`처럼 이 규칙의 버전을 나타냅니다. 규칙이 바뀔 때마다 함께 바뀝니다. 분석이
실패했거나, Actor가 분석을 건너뛰었거나, 기본 특성 답변이 없으면 점수는 `null`입니다. Actor는 빠진 점수를 추측으로 채우지
않습니다.
...
예/아니오로 비교됩니다. 판단은 분명하게 바뀐 경우에만 변경된 것으로 집계됩니다. 실행 간의 근소한 흔들림은 변경되지 않은 것으로 유지됩니다.
