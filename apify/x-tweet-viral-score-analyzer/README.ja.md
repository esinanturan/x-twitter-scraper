...
Xquikは世界最速かつ最安値のX(Twitter)スクレイパーサービスで、最も網羅的なXデータを提供します。X Tweet Viral Score Analyzerは、すべてのポストにViral Scoreの推定値と判定を付加します。他のApify Actorはすべて、フィルタリングや重複排除の前に課金します。Xquikは、配信済みでユニークかつフィルタ条件に一致する結果にのみ課金します。AI の費用はツイート単価に含まれています。AI プロバイダーへの支払いも、トークンの購入も、キーの用意も不要です。
...
- 監査できる、固定で公開済みの重みによる**ポストごとのViral Score**。
...
### ActorがViral Scoreを計算する方法

Actorは、0～2の各スコアを0から1の割合に換算します。次に、ポイントを加算します。

| 要素                                      | ポイント        |
| ----------------------------------------- | -------------- |
| フック                                     | 最大30         |
| 明確さ                                     | 最大20         |
| 見返り（情報性と面白さのうち高い方）           | 最大30         |
| 反応                                       | 最大20         |
| AI生成風の確率                              | 最大15の減点    |

反応は、その20ポイントのうちの割合を獲得します。共有は1、リプライは0.8、いいねは0.6、反論は0.4、無視は0です。その後、ハードストップがスコアに上限をかけます。スパムの確率が0.7以上の場合、上限は20です。レイジベイトの確率が0.7以上の場合、上限は35です。AI生成風の確率が0.8以上の場合、上限は60です。Actorは結果を整数に丸めます。
...
`viral.weights`は、`viral_lite:1`のように、これらのルールのバージョンを示します。重み、ストップ、しきい値のいずれかが変わるたびに、私たちはこのバージョンを上げます。分析が失敗した場合、Actorが分析をスキップした場合、またはデフォルトの特性への回答が欠けている場合、スコアは`null`です。Actorが欠けているスコアを推測で埋めることはありません。
...
- Actorは、メモリ使用量を一定に保つため、率を0.1幅のバケットにまとめます。同じバケットのポストは同順位として扱われるため、相関は近似値です。
...
Actorはポストを収集して配信しましたが、AI分析は完了しませんでした。`analysis.reason`が原因を示します。例えば、ポストとそのコンテキストが`maxContextBytes`を超えた場合の`context_limit`や、再試行後の`service_unavailable`などです。これらの行に結果料金は発生せず、スコアも付きません。`maxContextBytes`を上げる（最大12,000）か、対象のIDを再実行してください。
