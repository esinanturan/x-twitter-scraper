...
数据。X Tweet Viral Score Analyzer 为每条推文添加 Viral Score 估算值与一个结论。
其他所有 Apify Actor 都在过滤或去重之前就收费。Xquik 只为交付的、唯一的、
符合过滤条件的结果收费。AI 费用已包含在每条推文的价格中。你无需向 AI 提供商
付费、无需购买 token，也无需自带密钥。
...
- **每条帖子的 Viral Score**，基于固定且公开的权重，你可以审核。
...
### Actor 如何计算 Viral Score

Actor 将每个 0 到 2 的分数换算为 0 到 1 的比例。然后累加分值：

| 部分                             | 分值        |
| -------------------------------- | ----------- |
| 钩子                             | 最多 30     |
| 清晰度                           | 最多 20     |
| 回报，取信息量与趣味性中较高者   | 最多 30     |
| 反应                             | 最多 20     |
| AI 写作概率                      | 最多减 15   |

反应按比例获得其 20 分：分享 1，回复 0.8，点赞 0.6，争论 0.4，忽略 0。随后
硬性限制为分数封顶。垃圾内容概率达到 0.7 时，分数上限为 20。愤怒诱饵概率达到
0.7 时，分数上限为 35。AI 写作概率达到 0.8 时，分数上限为 60。Actor 将结果
四舍五入为整数。
...
`viral.weights` 标明这些规则的版本，例如 `viral_lite:1`。每当某个权重、限制或
阈值发生变化，我们都会提升该版本。分析失败、Actor 跳过分析或缺少某个默认特征
答案时，分数为 `null`。Actor 绝不会用猜测填补缺失的分数。
...
- Actor 将比率归入宽度为 0.1 的桶，以保持内存占用平稳。同一个桶中的帖子计为
  并列，因此相关系数是近似值。
...
数据集 ID）。此后每一行都会获得一个 `monitor` 对象。其状态在没有基线时为
`first_run`，更早运行中不存在的推文为 `new_to_baseline`，已存在的推文为
`unchanged` 或 `changed`。`changes` 列出每个从 `previous` 变为 `current` 的
特征判断。判断按类别、四舍五入的分数等级或 0.5 处的是否判断
进行比较。判断在三种情况下计为已更改。更早的类别概率降到 0.4 以下。分数移动
至少 0.6 个等级。是否概率与阈值相差至少 0.1。运行之间的临界抖动
...
Actor 已收集并交付该推文，但 AI 分析未能完成。`analysis.reason` 会说明
原因，例如推文及其上下文超过 `maxContextBytes` 时的 `context_limit`，或
重试后仍失败的 `service_unavailable`。这些行不产生结果费用，也没有分数。提高
